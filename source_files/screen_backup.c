...
void write_menu(container header, area body, container footer)
{
    ...
    if (body.dims.height > 0)
    {
        ...
        get_area(body_lines, body);
    }
    
    // FOOTER
    if (footer.dims.height > 0)
    {
        get_container(footer);
    }
}

void get_area(int height, area area_obj)
{
    ...
    */
    ...
    while ((written_lines < height) && (cont_index < area_obj.conts_quan))
    {
        ...
        // ); 
        get_container(area_obj.conts[cont_index]);
        cont_index++;
    }
    ...
    if (written_lines > 0)
    {
        ...
        {
            get_container(area_obj.conts[line]);
            // get_container(line, area_obj);
            // printf("\n");
        }
    }
    if (written_lines < height)
...
// char* get_container(char *cont_str, container cont_obj)
/* ??
...
*/
void get_container(container cont_obj)
{
...
void get_item(item item_obj, int index, index_type style) {
...
char* get_aligned(char *buffer, int width, char *text, alignment alignment, char symbol)
{
    switch (alignment)
    {
    case LEFT_TABBED:
